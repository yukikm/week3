...
const encrypt = async (
  ...
  const ciphertext: Ciphertext = {
        ...
        data: plaintext.map((e: BigInt, i: number): bigint => {
          let a = buf2Bigint(bigInt2Buffer(e))
          let b = buf2Bigint(mimc7.hash(
            sharedKey,
            iv + BigInt(i),
        ))
            return a+b
...
const decrypt = async (
  ...
  const plaintext: Plaintext = ciphertext.data.map(
    (e: BigInt, i: number): bigint => {
        return buf2Bigint(bigInt2Buffer(e)) - buf2Bigint(mimc7.hash(sharedKey, BigInt(buf2Bigint(bigInt2Buffer(ciphertext.iv))) + BigInt(i)))
    }
  )
